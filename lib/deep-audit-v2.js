...
function deepAudit() {
    ...
    const worldwideMcCampaigns = Array.from(metricsAggregated.entries())
        ...
        return WORLDWIDE_MCDONALD_BUGS.some(bugName => name.includes(bugName.toLowerCase()));
    })
        .map(([id, data]) => ({
        id,
        name: data.name,
    ...
    const outputPath = path_1.default.join(process.cwd(), 'data - v2', 'deep_audit_v2_results.json');
